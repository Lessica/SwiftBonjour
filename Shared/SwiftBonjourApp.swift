...
struct SwiftBonjourApp: App {
    let server = BonjourServer(type: ServiceType.tcp(String(describing: SwiftBonjourApp.self)))
    ...
    static let displayHardwareModel: String = {
        func displayNameForHardwareModel(_ identifier: String) -> String { // swiftlint:disable:this cyclomatic_complexity
            switch identifier {
        ...
        return displayNameForHardwareModel(hardwareModel)
    }()
            ...
            ContentView()
                .onAppear {
                    server.start { succeed in
                        print("Bonour server started: ", succeed)
                    }
                    
                    server.txtRecord = [
                        "hwmodel": SwiftBonjourApp.hardwareModel,
                    ...
                    browser.browse(type: server.serviceType)
                }
                .onDisappear {
                    server.stop()
                }
